...
#define MAX_DEVNUM      5
const char* dev[MAX_DEVNUM] = {"/dev/secbulk0", "/dev/secbulk1",
                                "/dev/secbulk2", "/dev/secbulk3", "/dev/secbulk4"};
...
static int _download_buffer(struct download_buffer *buf)
{
    ...
    for (devnum = 0; devnum < MAX_DEVNUM; devnum++) {
	    fd_dev = open(dev[devnum], O_WRONLY);
	    if( -1 == fd_dev) {
